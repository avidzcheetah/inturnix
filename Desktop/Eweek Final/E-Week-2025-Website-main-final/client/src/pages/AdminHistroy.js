...
const AdminHistory = () => {
  ...
  const fetchAdminData = async () => {
    ...
    const res = await fetch("http://localhost:5000/api/EweekLogin/admin", {
  ...
  const AllHistroy = async () => {
    try {
      const response = await fetch("http://localhost:5000/api/EweekHistroy/getEweekHistroy");
  ...
  const handleDeleteYear = async (_id) => {
    try {
      const response = await fetch("http://localhost:5000/api/EweekHistroy/deleteEweekHistoryById", {
        method: "POST",
        headers: {
